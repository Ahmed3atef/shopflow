...
class SalesOrders(Document):
	...
	def create_store_ledger_entries(self) -> None:
		for item in self.items:
			sle = frappe.get_doc(
				{
					...
					"remarks": "Stock deducted on order confirmation",
				}
			)
			sle.insert(ignore_permissions=True)

	def reverse_stock_ledger_entries(self) -> None:
		for item in self.items:
			sle = frappe.get_doc(
				{
					"doctype": "Stock Ledger Entry s",
					...
					"voucher_type": "Sales Order s",
					"voucher_no": self.name,
					"remarks": "Stock reversed on order cancellation",
				}
			)
			sle.insert(ignore_permissions=True)
